...
- The only automated verification is that the solution builds: `dotnet build`
  (see Build command in Tools). No test suites, no automated game runs.
- **Acceptance is performed by the user**, via manual in-game runs. The user
  reports the outcome; do not treat a successful build as proof the mod works.

# Working style

- Do not fear being wrong. If you are not 100% certain how to write a patch but
  have a sound working hypothesis from the research, **write it anyway** — it
  will be refined together with the user afterwards.
- Do not spin in circles for hours: once the research (decompiled sources, docs,
  game behavior) has provided the needed information, stop researching and
