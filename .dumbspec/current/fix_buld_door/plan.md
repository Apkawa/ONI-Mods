...
## Stage 3 — Sandbox: instant wall→door replacement (RESUMED 2026-09-05 — user confirmed survival fully working)
...
- [x] Red: independent trace of `BuildTool.TryBuild` in InstantBuild mode for the door def over a FoundationTile / Backwall occupant; assert each fact line-by-line against Assembly-CSharp; record every gap (does the normal gate at BuildTool.cs:325 still pass over a foundation so `def.Build` fires instead of the replacement fallback? what does the multi-cell branch of `InstantBuildReplace` cover/miss?) → `.tmp/stage3_instant_trace.md`

**Red findings (2026-09-05, `.tmp/stage3_instant_trace.md`):**
1. The instant gate (BuildTool.cs:325) calls the 4-arg `IsValidPlaceLocation` (BuildingDef.cs:1098) — exactly the overload the mod patches — so the P1 flip fires behaviorally in instant mode: gate passes → `def.Build` (BuildTool.cs:348) builds the door ON TOP of the live wall(s) (MarkArea overwrites layer-1 entries, BuildingDef.cs:842; walls survive in layer 9 with solid sim cells); the replacement fallback / `InstantBuildReplace` is never reached. (GAP 1 — the flip's "cosmetic only" comments, Mod.cs:209-212/:319-324, are wrong for the instant branch.)
2. Wall in the UPPER cell only: anchor-only `GetReplacementCandidate` (BuildTool.cs:352) misses it; the P3 postfix bails in instant mode → nothing builds, wall alive under the door (GAP 2).
3. Wall PLAN occupant: plans lack `BuildingComplete` (BuildingLoader.cs:49-59) → rejected by every candidate gate → nothing happens, plan not cancelled (GAP 3). Designed cancel = `Trigger(2127324410)` (GameHashes.Cancel) → Cancellable.OnCancel + Constructable.OnCancel (ClearMaterialNeeds + ClearPendingUproots); raw Destroy leaks MaterialNeeds + pending uproots.
4. Backwall occupant: layer-1 check never fails for layer-2 occupants → gate natively true → `def.Build` over live backwall, no destruction path (GAP 4).
5. Native baseline (flip removed): the `InstantBuildReplace` multi-cell branch (BuildTool.cs:392-431) already correctly destroys both walls + builds for the both-cells-over-wall geometry — the flip is what diverts instant mode away from it.

**Note (2026-09-05):** user in-game commit `d6de110` fixed a rotation edge case in the P3 orientation-mirror branch (doors with `def.TileLayer=FoundationTile`, e.g. Manual Airlock — the door flipped in place instead of moving; fix: `anchorCell = Grid.CellAbove/CellRight(__0)` instead of `anchorCell = candidateCell`). Stage-3 work builds on top of it — that branch is LIVE code for those door defs, not dead code.

**Note (2026-09-05):** user in-game commit `d6de110` fixed a rotation edge case in the P3 orientation-mirror branch (doors with `def.TileLayer=FoundationTile`, e.g. Manual Airlock — door flipped in place instead of moving; fix: `anchorCell = Grid.CellAbove/CellRight(__0)` instead of `anchorCell = candidateCell`). Stage-3 work builds on top of it.

**Stage-3 design (settled from the trace):** keep the P1 flip as-is (keeps instant hover tint green; the gate-driven `def.Build` over candidates becomes intentional — the door is placed, then the P3 `TryBuild` postfix gains an INSTANT branch that destroys the surviving candidates (sim-safe deferred `DestroySelf` like InstantBuildReplace) and, when native built nothing (plan case), cancels plans via `Trigger(2127324410)` before building the door itself mirroring InstantBuildReplace's anchor tail (BuildTool.cs:417-430). P3 survival branch stays byte-identical. Survival blueprint-under-door case remains next-iteration scope (user journal).

**Execution mode (user instruction 2026-09-05):** implement straightforward, no deep analysis rounds; independent acceptance audit SKIPPED at the user's request — the user verifies in-game and polishes afterwards ("пусть как получится, я проверю и допилю"). Also: user in-game commit `d6de110` fixed a rotation edge case in the P3 orientation-mirror branch (doors with `def.TileLayer=FoundationTile`, e.g. Manual Airlock — door flipped in place instead of moving; fix: `anchorCell = Grid.CellAbove/CellRight(__0)` instead of `anchorCell = candidateCell`). Stage-3 work builds on top of it — that branch is LIVE code for those door defs, not dead code.
...
- [~] Green: static check + full build (independent re-trace deferred — user verifies in-game and polishes; see note below)
