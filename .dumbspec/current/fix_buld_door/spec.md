...
## Статус по игре (ручная проверка пользователя после Stage 1, коммит `ee4609b`)

- ✅ **В режиме выживания** дверь заменяет собой стену и гипсокартон (Backwall) — нативный replacement-flow работает на одном только metadata-патче Stage 1 (гейты `Replaceable`/`CanReplace` имплицитно прошли).
- ⚠️ **Осталась проблема:** при **наведении** двери на стену/гипсокартон во время строительства показывается предупреждение «нужно свободное место строительства», хотя план размещается корректно. Нужно найти, где выводятся такие предупреждения, и устранить ложный алерт. Подсказка от пользователя: смотреть в `UpdateVis` (сообщения при сборке/наведении).

**Приоритет (решение пользователя):** сначала довести режим выживания (включая фикс предупреждения); песочница — отдельный отложенный этап.

## Открытые вопросы (решаются на этапах реализации, см. plan.md)

1. Где именно формируется предупреждение «свободное место строительства» при наведении (метод визуализатора `BuildTool` — вероятно `UpdateVis`/`UpdateVisualizer`; `fail_reason` из `IsValidPlaceLocation` без replacement-флага; цвет превью через `IsValidReplaceLocation`, BuildTool.cs:178) — и почему оно срабатывает, хотя replacement-валидация (`replace_tile=true`) в той же клетке проходит?
2. Как минимально показать валидное превью при наличии заменяемого кандидата, **не отводя** drag-путь от replacement-fallback (в survival дверь должна продолжать идти через `TryReplaceTile`, а не через обычный `TryPlace`).
3. (Отложено до этапа песочницы) `BuildingComplete` у кандидата-плана `BuildingUnderConstruction` — для бага 2 (план стены под дверью).
